...
//
// if(!isset($GLOBALS['_cookies'])){
	// $GLOBALS['_cookies'] = $_cookies;
// } 
$_config            = array
                    (
                        'url_var_name'             => 'q',
...
function add_cookie($name, $value, $expires = 0)
{
    ...
    // return $name . '=' . rawurlencode($value) . (empty($expires) ? '' : '; expires=' . gmdate('D, d-M-Y H:i:s \G\M\T', $expires)) . '; path=/; domain=.' . $GLOBALS['_http_host'];
}
